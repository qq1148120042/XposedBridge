...
import static com.android.ks.tool.ShadowHelpers.fileContains;
...
import static com.android.ks.tool.ShadowHelpers.getBooleanField;
import static com.android.ks.tool.ShadowHelpers.getObjectField;
import static com.android.ks.tool.ShadowHelpers.getOverriddenMethods;
...
/*package*/ final class KsInit {
	private static final String TAG = ShadowConnect.TAG;

	private static final boolean startsSystemServer = ShadowConnect.startsSystemServer();
	private static final String startClassName = ShadowConnect.getStartClassName();

	private static final String INSTALLER_PACKAGE_NAME = "com.android.ks.tool.installer";
		...
		if (needsToCloseFilesForFork()) {
			XC_MethodHook callback = new XC_MethodHook() {
				...
				protected void beginRunning(MethodHookParam param) throws Throwable {
					ShadowConnect.closeFilesBeforeForkNative();
				}

				@Override
				protected void endRunning(MethodHookParam param) throws Throwable {
					ShadowConnect.reopenFilesAfterForkNative();
				}
			};
			...
			findAndHookMethod("com.android.server.ServerThread", null,
					Build.VERSION.SDK_INT < 19 ? "run" : "initAndLoop", new XC_MethodHook() {
						...
						protected void beginRunning(MethodHookParam param) throws Throwable {
							...
							lpparam.classLoader = ShadowConnect.BOOTCLASSLOADER;
	...
	/*package*/ static void hookResources() throws Throwable {
		...
		if (!ShadowConnect.initXResourcesNative()) {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			...
			ShadowConnect.invalidateCallersNative(methods.toArray(new Member[methods.size()]));
		}
		...
		hookAllConstructors(TypedArray.class, new XC_MethodHook() {
			...
			protected void endRunning(MethodHookParam param) throws Throwable {
				...
				if (res instanceof XResources) {
					ShadowConnect.setObjectClass(typedArray, XResources.XTypedArray.class);
				}
			}
		});

		// Replace system resources
		XResources systemRes = (XResources) ShadowConnect.cloneToSubclass(Resources.getSystem(), XResources.class);
	...
	private static XResources cloneToXResources(XC_MethodHook.MethodHookParam param, String resDir) {
		...
		XResources newRes = (XResources) ShadowConnect.cloneToSubclass(result, XResources.class);
	...
	private static void loadModule(String apk, ClassLoader topClassLoader) {
		...
		if (dexFile.loadClass(ShadowConnect.class.getName(), topClassLoader) != null) {
		...
		ClassLoader mcl = new PathClassLoader(apk, ShadowConnect.BOOTCLASSLOADER);
				...
				try {
					...
					if (ShadowConnect.isZygote) {
						...
						if (moduleInstance instanceof IXposedHookLoadPackage)
							ShadowConnect.hookLoadPackage(new IXposedHookLoadPackage.Wrapper((IXposedHookLoadPackage) moduleInstance));

						if (moduleInstance instanceof IXposedHookInitPackageResources)
							ShadowConnect.hookInitPackageResources(new IXposedHookInitPackageResources.Wrapper((IXposedHookInitPackageResources) moduleInstance));
					} else {
